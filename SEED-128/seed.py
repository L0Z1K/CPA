...
c = b"\xB8\xFD\x43\x7D\xDA\x21\x9A\x39\x61\x94\x3C\xD1\x34\x5D\x28\xFD\x1E\x17\xB7\x44\x57\x21\x44\xA1\x84\xF1\x30\xCE\x9C\x07\xD9\x4E"
key = open("key.txt", "rb").read().split("\n")[:-1]
for i in range(len(key)):
    realkey = struct.pack(">Q",int(key[i][:16], 16))+struct.pack(">Q",int(key[i][16:], 16))
    p = decrypt(realkey, c)
    if "\x02\x02" in p:
        print(p)
